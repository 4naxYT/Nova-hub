...
-- ══════════════════════════════════════════
--  CIRCLE
--  Uses UICorner(1,0) for shape + UIStroke for hollow
--  NovaDraw.circle(x, y, radius, thickness, color, filled, opacity)
--  x,y = center
-- ══════════════════════════════════════════
...
--  SQUARE / RECTANGLE
--  Filled = single Frame
--  Hollow = 4 border Frames
--  Rounded variant uses UICorner + UIStroke
--  NovaDraw.square(x, y, sizeX, sizeY, thickness, color, filled, opacity, cornerRadius)
--  x,y = top-left
-- ══════════════════════════════════════════
...
function NovaDraw.square(x, y, sizeX, sizeY, thickness, color, filled, opacity, cornerRadius)
    ...
    if cornerRadius > 0 then
        -- use UICorner + UIStroke for rounded variant
        local f = newFrame()
...
-- ══════════════════════════════════════════
--  LINE
--  Single rotated Frame, width = distance, height = thickness
--  Rounded ends via UICorner(0, thickness/2)
--  NovaDraw.line(x1,y1, x2,y2, thickness, color, opacity, rounded)
-- ══════════════════════════════════════════
...
-- ══════════════════════════════════════════
--  TEXT
--  TextLabel inside transparent Frame container
--  Optional 4-direction shadow for outline effect
--  NovaDraw.text(x, y, content, size, color, opacity, outline)
-- ══════════════════════════════════════════
...
-- ══════════════════════════════════════════
--  POLYGON
--  N-sided polygon as N rounded lines
--  NovaDraw.polygon(x, y, sides, radius, thickness, color, opacity)
-- ══════════════════════════════════════════
...
function NovaDraw.crosshair(x, y, length, gap, thickness, color, opacity)
    ...
    for i=1,4 do table.remove(NovaDraw._objects,#NovaDraw._objects) end
    return register({ _type="crosshair", _h=h, _h2=h2, _v=v, _v2=v2, _gap=gap })
end
...
function NovaDraw.healthBar(x, y, w, h, percent, fillColor, opacity)
    ...
    local fill  = newFrame()
    setAbsolute(fill, x, y + (h - fillH), w, fillH)
    fill.BackgroundColor3       = fillColor
    fill.BackgroundTransparency = 1 - opacity
    local fillCorner = Instance.new("UICorner", fill)
    fillCorner.CornerRadius = UDim.new(0, 3)

    -- outline via UIStroke on bg
...
function NovaDraw.progressBar(x, y, w, h, percent, fillColor, bgColor, opacity)
    ...
    bg.BackgroundTransparency = 1 - (opacity * 0.7)
    local bgC = Instance.new("UICorner", bg); bgC.CornerRadius = UDim.new(0, math.floor(h/2))
    ...
    fill.BackgroundColor3       = fillColor
    fill.BackgroundTransparency = 1 - opacity
    local fillC = Instance.new("UICorner", fill); fillC.CornerRadius = UDim.new(0, math.floor(h/2))
...
function NovaDraw.moveCrosshair(obj, x, y)
    local r   = (obj._h._x2 - obj._h._x1) / 2
    local gap = obj._gap or 0
    NovaDraw.moveLine(obj._h,  x-r-gap, y,   x-gap,   y)
    NovaDraw.moveLine(obj._h2, x+gap,   y,   x+r+gap, y)
    NovaDraw.moveLine(obj._v,  x, y-r-gap,   x, y-gap)
    NovaDraw.moveLine(obj._v2, x, y+gap,     x, y+r+gap)
end
...
function NovaDraw.setThickness(obj, thickness)
    if obj._type == "circle" and obj._stroke then
...
function NovaDraw.setVisible(obj, v)
    if obj._frame      then obj._frame.Visible=v
    elseif obj._frames then for _,f in ipairs(obj._frames) do f.Visible=v end
    elseif obj._container then obj._container.Visible=v
    elseif obj._lines  then for _,l in ipairs(obj._lines) do NovaDraw.setVisible(l,v) end
    end
    if obj._type=="crosshair" then
        NovaDraw.setVisible(obj._h,v);NovaDraw.setVisible(obj._h2,v)
        NovaDraw.setVisible(obj._v,v);NovaDraw.setVisible(obj._v2,v)
    elseif obj._type=="arrow" then
        NovaDraw.setVisible(obj._body,v);NovaDraw.setVisible(obj._h1,v);NovaDraw.setVisible(obj._h2,v)
    elseif obj._type=="healthBar" then
        obj._bg.Visible=v; obj._fill.Visible=v
    elseif obj._type=="progressBar" then
        obj._bg.Visible=v; obj._fill.Visible=v
    end
end
...
local function destroyObj(obj)
    ...
    if obj._frames    then for _,f in ipairs(obj._frames)   do f:Destroy() end end
    if obj._container then obj._container:Destroy() end
    if obj._lines     then for _,l in ipairs(obj._lines)    do destroyObj(l) end end
    if obj._type=="crosshair" then
        destroyObj(obj._h); destroyObj(obj._h2)
        destroyObj(obj._v); destroyObj(obj._v2)
    end
    if obj._type=="arrow" then
        destroyObj(obj._body); destroyObj(obj._h1); destroyObj(obj._h2)
    end
    if obj._type=="healthBar"   then obj._bg:Destroy(); obj._fill:Destroy() end
    if obj._type=="progressBar" then obj._bg:Destroy(); obj._fill:Destroy() end
end

function NovaDraw.remove(obj)
    pcall(destroyObj, obj)
    for i, entry in ipairs(NovaDraw._objects) do
        if entry == obj then table.remove(NovaDraw._objects, i); break end
    end
end

function NovaDraw.clearAll()
    for _, obj in ipairs(NovaDraw._objects) do pcall(destroyObj, obj) end
    NovaDraw._objects = {}
end
